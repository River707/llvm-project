...
///
///  * Explicitly via `DECLARE_EXPLICIT_TYPE_ID` and `DEFINE_EXPLICIT_TYPE_ID`
///
...
///
///        // Foo.h
///        DECLARE_EXPLICIT_TYPE_ID(Foo);
///
///        // Foo.cpp
///        DEFINE_EXPLICIT_TYPE_ID(Foo);
///
///  * Explicitly via `DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID`
///   - This methods explicitly defines the type ID for a given type by
...
///       class Foo {
///       public:
///         DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(Foo)
///       };
///       } // namespace
///
...
/// allows for the derived type to specialize it's resolution behavior. The
...
#define DEFINE_EXPLICIT_TYPE_ID(CLASS_NAME)                                    \
  namespace mlir {                                                             \
  namespace detail {                                                           \
  TypeIDResolver<CLASS_NAME>::TypeIDResolver() {}                              \
...
#define DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID(CLASS_NAME)                    \
  ...
  static_assert(                                                               \
      ::mlir::detail::InlineTypeIDResolver::has_resolve_typeid<                \
          CLASS_NAME>::value,                                                  \
      "`DEFINE_EXPLICIT_INTERNAL_INLINE_TYPE_ID` must be placed in a "         \
