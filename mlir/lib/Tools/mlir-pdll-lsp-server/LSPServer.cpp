...
#include "../lsp-server/Transport.h"
...
struct LSPServer::Impl {
  Impl(PDLLServer &server, JSONTransport &transport)
...
LSPServer::LSPServer(PDLLServer &server, JSONTransport &transport)
...
LogicalResult LSPServer::run() {
  ...
  return success(impl->shutdownRequestReceived);
}
