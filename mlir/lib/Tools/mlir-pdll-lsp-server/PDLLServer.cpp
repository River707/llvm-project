...
#include "../lsp-server/Logging.h"
#include "../lsp-server/Protocol.h"
...
static lsp::URIForFile getURIFromLoc(llvm::SourceMgr &mgr, llvm::SMRange loc,
...
static lsp::Location getLocationFromLoc(llvm::SourceMgr &mgr,
  ...
  return lsp::Location{getURIFromLoc(mgr, range, uri),
                       getRangeFromLoc(mgr, range)};
}
...
struct PDLIndexSymbol {
  ...
  std::vector<llvm::SMRange> references;
};
...
public:
  ...
  const PDLIndexSymbol *lookup(llvm::SMLoc loc,
...
void PDLIndex::initialize(const ast::Module &module,
  ...
  auto insertDeclRef = [&](PDLIndexSymbol *sym, llvm::SMRange refLoc,
                           bool isDef = false) {
  ...
  auto insertODSOpRef = [&](StringRef opName, llvm::SMRange refLoc) {
...
const PDLIndexSymbol *PDLIndex::lookup(llvm::SMLoc loc,
...
void PDLDocument::findReferencesOf(const lsp::URIForFile &uri,
  ...
  for (llvm::SMRange refLoc : symbol->references)
...
public:
  LSPCodeCompleteContext(llvm::SMLoc completeLoc,
  ...
  void codeCompleteConstraintName(ast::Type currentType,
    ...
    auto addCoreConstraint = [&](StringRef constraint, StringRef mlirType,
      ...
      item.documentation = lsp::MarkupContent{
          lsp::MarkupKind::Markdown,
          ("A single entity core constraint corresponding to a `" + mlirType +
           "`")
              .str()};
      item.sortText = "0";
    ...
    while (scope) {
      for (const ast::Decl *decl : scope->getDecls()) {
        if (const auto *cst = dyn_cast<ast::UserConstraintDecl>(decl)) {
          ...
          lsp::CompletionItem item;
          item.label = cst->getName().getName();
...
PDLDocument::getCodeCompletion(const lsp::URIForFile &uri,
                               const lsp::Position &completePos) {
  llvm::SMLoc posLoc = getPosFromLoc(sourceMgr, completePos);
  if (!posLoc.isValid())
    return lsp::CompletionList();

  // Adjust the position one further to after the completion trigger token.
  posLoc = llvm::SMLoc::getFromPointer(posLoc.getPointer() + 1);
...
public:
  LSPSignatureHelpContext(llvm::SMLoc completeLoc,
...
lsp::SignatureHelp PDLDocument::getSignatureHelp(const lsp::URIForFile &uri,
                                                 const lsp::Position &helpPos) {
  llvm::SMLoc posLoc = getPosFromLoc(sourceMgr, helpPos);
  if (!posLoc.isValid())
