...
export function activate(mlirContext: MLIRContext) {
  ...
  mlirContext.subscriptions.push(vscode.workspace.onDidChangeConfiguration(event => {
    const settings: string[] = [ 'server_path', 'pdl_server_path' ];
  ...
  const servers: string[][] = [
    [ 'server_path', "mlir-lsp-server" ],
    [ 'pdl_server_path', "pdl-lsp-server" ]
  ];
  for (const server in servers) {
    const userDefinedServerPath = config.get<string>(server[0]);
    const serverPath = path.resolve(
        (userDefinedServerPath === '') ? server[1] : userDefinedServerPath);
