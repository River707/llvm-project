...
struct PrintOpAvailability
    : public PassWrapper<PrintOpAvailability, SymbolDefinitionPass<FuncOp>> {
...
void PrintOpAvailability::runOnSymbol() {
  auto f = getOperation();
  llvm::outs() << f.getName() << "\n";
...
struct ConvertToTargetEnv
    : public PassWrapper<ConvertToTargetEnv, SymbolDefinitionPass<FuncOp>> {
...
void ConvertToTargetEnv::runOnSymbol() {
